...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let artworkUrl = `data:image/jpeg;base64,${base64ImageBytes}`;
    
    // For non-test runs, upload to Blob storage and save to KV.
    if (!isTestRun) {
        if (!process.env.BLOB_READ_WRITE_TOKEN) {
            console.warn("BLOB_READ_WRITE_TOKEN is not set. Skipping gallery save. The image will be temporary.");
        } else {
            const imageBuffer = Buffer.from(base64ImageBytes, 'base64');
            const uniqueId = `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
            const filename = `gallery/${uniqueId}.jpeg`;
